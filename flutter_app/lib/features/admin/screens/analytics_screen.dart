...
// ══════════════════════════════════════════════════════════════════════════════
// 1. Monthly Line Chart — courbe with months on x-axis
// ══════════════════════════════════════════════════════════════════════════════
...
class _MonthlyLineChart extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Fill all 12 months — months with no data get 0
    ...
    for (var m = 1; m <= 12; m++) {
      final row = byMonth[m];
      final x = (m - 1).toDouble(); // 0–11 on the axis
...
// ══════════════════════════════════════════════════════════════════════════════
// 2. Horizontal Bar Chart — reusable for both agents and forests
// ══════════════════════════════════════════════════════════════════════════════

class _HBarEntry {
  final String label;
  final int value;
  final int sub; // secondary count shown in the badge (resolved / critical)
  const _HBarEntry({
    required this.label,
    required this.value,
    required this.sub,
...
class _PeakHoursHeatmap extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // ── Build a 7×24 lookup map: (dow, hour) → count ──────────────────
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                const SizedBox(width: 36), // left gutter for day labels
                Expanded(
                  child: Row(
                    children: List.generate(24, (h) {
                      // Only label every 3rd hour to avoid crowding
                      return Expanded(
                        child: Text(
                          h % 3 == 0 ? '$h' : '',
            ...
            Row(
              ...
              children: [
                ...
                ...List.generate(5, (i) {
                  ...
                  final label = i == 4
                      ? '$lo' // last step just shows the max
                      : '$lo–$hi';
  ...
  /// Maps a 0.0–1.0 intensity to a colour from light green → orange → red.
  Color _cellColor(double t) {
    if (t == 0) return Colors.grey.withValues(alpha: 0.08); // empty cell
    if (t < 0.5) {
      return Color.lerp(
